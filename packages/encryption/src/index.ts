...
function getKey(): Buffer {
  const key = process.env.CONNORCO_ENCRYPTION_KEY;
  if (!key) {
    throw new Error("CONNORCO_ENCRYPTION_KEY environment variable is not set.");
  }
  if (Buffer.from(key, "hex").length !== 32) {
    throw new Error(
      "CONNORCO_ENCRYPTION_KEY must be a 64-character hex string (32 bytes).",
    );
  }
