import { decryptOAuthState, encryptOAuthState } from "@connorco/encryption";
 ...
 *
 * @param transactionId - The Connor & Co transaction ID
 ...
 * // Returns: "connorandco-tx-tx-123-job-456"
 */
export function generateTransactionIdempotencyKey(
  transactionId: string,
  jobId: string,
): string {
  return `connorandco-tx-${transactionId}-${jobId}`;
}
 ...
 * // Returns: "connorandco-attachment-abc-123-receipt.pdf"
 */
export function generateAttachmentIdempotencyKey(
  providerTransactionId: string,
  fileName: string,
): string {
  return `connorandco-attachment-${providerTransactionId}-${fileName}`;
}
