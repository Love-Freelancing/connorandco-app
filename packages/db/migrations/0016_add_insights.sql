...
CREATE UNIQUE INDEX IF NOT EXISTS insights_team_period_unique 
    ON insights(team_id, period_type, period_year, period_number);
CREATE INDEX IF NOT EXISTS insights_team_id_idx ON insights(team_id);
CREATE INDEX IF NOT EXISTS insights_team_period_type_idx 
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      ...
      AND policyname = 'Team members can view their insights'
  ) THEN
    IF EXISTS (
      SELECT 1
      FROM pg_proc p
...
CREATE TABLE IF NOT EXISTS insight_user_status (
...
CREATE INDEX IF NOT EXISTS insight_user_status_user_idx ON insight_user_status(user_id);
CREATE INDEX IF NOT EXISTS insight_user_status_insight_idx ON insight_user_status(insight_id);
CREATE INDEX IF NOT EXISTS insight_user_status_user_dismissed_idx 
    ON insight_user_status(user_id, dismissed_at) 
    WHERE dismissed_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS insight_user_status_unread_idx 
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'insight_user_status'
      AND policyname = 'Users can view their own insight status'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'insight_user_status'
      AND policyname = 'Users can insert their own insight status'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'insight_user_status'
      AND policyname = 'Users can update their own insight status'
  ) THEN
