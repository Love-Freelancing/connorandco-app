...
const projectTotalDurationSql = sql<number>`COALESCE((
  SELECT SUM(${trackerEntries.duration})
  FROM ${trackerEntries}
  WHERE ${trackerEntries.projectId} = ${trackerProjects.id}
), 0)`;

const projectTotalAmountSql = sql<number>`COALESCE((
  SELECT SUM((COALESCE(${trackerEntries.duration}, 0)::numeric / 3600) * COALESCE(${trackerProjects.rate}, 0)::numeric)
  FROM ${trackerEntries}
  WHERE ${trackerEntries.projectId} = ${trackerProjects.id}
), 0)::float`;
...
export async function getTrackerProjects(
  ...
  const query = db
    .select({
      ...
      totalDuration: projectTotalDurationSql.as("total_duration"),
      totalAmount: projectTotalAmountSql.as("total_amount"),
      customer: {
        id: customers.id,
        name: customers.name,
