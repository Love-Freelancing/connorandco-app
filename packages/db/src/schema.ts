...
export const clientPortalMessageSenderEnum = pgEnum(
  "client_portal_message_sender",
  ["client", "freelancer"],
);
...
export const customers = pgTable(
  "customers",
  {
    ...
    name: text().notNull(),
    email: text().notNull(),
    // Legacy column is quoted camelCase in existing databases.
    ...
    portalBillingType: text("portal_billing_type").default("subscription"),
    portalProjectName: text("portal_project_name"),
    portalProjectTotal: text("portal_project_total"),
    portalHideSubscriptionCta: boolean("portal_hide_subscription_cta").default(
      false,
    ),
...
export const clientRequests = pgTable(
  "client_requests",
  {
    ...
    title: text().notNull(),
    details: text(),
    status: clientRequestStatusEnum("status").default("backlog").notNull(),
  ...
  (table) => [
    index("client_requests_team_id_idx").on(table.teamId),
    index("client_requests_customer_id_idx").on(table.customerId),
    index("client_requests_customer_priority_idx").on(
      table.customerId,
      table.priority,
    ),
    foreignKey({
      columns: [table.teamId],
      foreignColumns: [teams.id],
      name: "client_requests_team_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.customerId],
      foreignColumns: [customers.id],
      name: "client_requests_customer_id_fkey",
    }).onDelete("cascade"),
    pgPolicy("Client requests can be handled by a member of the team", {
      as: "permissive",
      for: "all",
      to: ["public"],
      using: sql`(
        team_id IN ( SELECT private.get_teams_for_authenticated_user() AS get_teams_for_authenticated_user)
...
export const clientPortalMessages = pgTable(
  "client_portal_messages",
  {
    ...
    senderType: clientPortalMessageSenderEnum("sender_type").notNull(),
  ...
  (table) => [
    index("client_portal_messages_team_id_idx").on(table.teamId),
    index("client_portal_messages_customer_id_idx").on(table.customerId),
    index("client_portal_messages_request_id_idx").on(table.requestId),
    foreignKey({
      columns: [table.teamId],
      foreignColumns: [teams.id],
      name: "client_portal_messages_team_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.customerId],
      foreignColumns: [customers.id],
      name: "client_portal_messages_customer_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.requestId],
      foreignColumns: [clientRequests.id],
      name: "client_portal_messages_request_id_fkey",
    }).onDelete("set null"),
    foreignKey({
      columns: [table.senderUserId],
      foreignColumns: [users.id],
      name: "client_portal_messages_sender_user_id_fkey",
    }).onDelete("set null"),
    pgPolicy("Client portal messages can be handled by a member of the team", {
      as: "permissive",
      for: "all",
      to: ["public"],
      using: sql`(
...
export const clientRequestsRelations = relations(clientRequests, ({ one }) => ({
  customer: one(customers, {
    fields: [clientRequests.customerId],
    references: [customers.id],
  }),
  team: one(teams, {
    fields: [clientRequests.teamId],
    references: [teams.id],
  }),
}));
...
export const accountingSyncRecords = pgTable(
  "accounting_sync_records",
  {
    ...
    // Maps Connor & Co attachment IDs to provider attachment IDs for sync tracking
    // Format: { "connorandco-attachment-id": "provider-attachment-id" }
