 ...
 * Looks for patterns like "inbox ID is: xxx", "Connect to Connor & Co: xxx", or just the inbox ID
 */
export function extractInboxIdFromMessage(text: string): string | null {
  ...
  // Pattern 1: "Connect to Connor & Co: xxx" (case-insensitive, fixed spacing to avoid ReDoS)
  // Matches: "Connect to Connor & Co: abc123", "connect to connorandco:abc123", etc.
  // Using [ \t]+ instead of \s+ and avoiding ambiguous quantifiers to prevent backtracking
  const patternConnect =
    /connect[ \t]+to[ \t]+connorandco[ \t]*:?[ \t]*([a-zA-Z0-9]+)/i;
