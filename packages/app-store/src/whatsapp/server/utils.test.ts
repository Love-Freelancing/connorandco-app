...
describe("extractInboxIdFromMessage", () => {
  describe("Pattern 1: 'Connect to Connor & Co: xxx'", () => {
    test("should extract inbox ID from standard format", () => {
      expect(extractInboxIdFromMessage("Connect to Connor & Co: abc123")).toBe(
        "abc123",
      );
    });

    test("should extract inbox ID with case-insensitive matching", () => {
      expect(extractInboxIdFromMessage("connect to connorandco: xyz789")).toBe(
        "xyz789",
      );
      expect(extractInboxIdFromMessage("CONNECT TO CONNORCO: def456")).toBe(
        "def456",
      );
      expect(extractInboxIdFromMessage("CoNnEcT tO connorandco: ghi012")).toBe(
        "ghi012",
      );
    });
    ...
    test("should extract inbox ID with leading/trailing whitespace", () => {
      ...
      expect(extractInboxIdFromMessage("\tConnect to Connor & Co: abc123\n")).toBe(
        "abc123",
      );
    });
    ...
    test("should handle longer inbox IDs", () => {
      expect(
        extractInboxIdFromMessage("Connect to Connor & Co: abc123def456ghi789"),
  ...
  describe("Edge cases and invalid inputs", () => {
    ...
    test("should return null for messages with special characters", () => {
      expect(extractInboxIdFromMessage("Connect to Connor & Co: abc-123")).toBe(
        "abc",
      ); // Only matches up to special char
      expect(extractInboxIdFromMessage("Connect to Connor & Co: abc_123")).toBe(
        "abc",
      );
  ...
  describe("Real-world scenarios", () => {
    test("should handle the exact format generated by the client", () => {
      ...
      const message = `Connect to Connor & Co: ${inboxId}`;
    ...
    test("should prioritize Pattern 1 over Pattern 3", () => {
      ...
      expect(extractInboxIdFromMessage("Connect to Connor & Co: abc")).toBe("abc");
    });

    test("should handle messages with extra text before pattern", () => {
      expect(extractInboxIdFromMessage("Hi! Connect to Connor & Co: abc123")).toBe(
        "abc123",
      );
    });
    ...
    test("should handle messages with extra text after pattern", () => {
      ...
      expect(
        extractInboxIdFromMessage("Connect to Connor & Co: abc123 thanks!"),
      ).toBe("abc123");
    });
  });
});
