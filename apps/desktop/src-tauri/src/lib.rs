...
async fn create_preloaded_search_window(
    ...
    .title("Connor & Co Search")
    .inner_size(720.0, 450.0)
    .min_inner_size(720.0, 450.0)
    .resizable(false)
    .user_agent("Mozilla/5.0 (compatible; Connor & Co Desktop App)")
    .transparent(true)
    .decorations(false)
...
fn get_app_url() -> String {
    ...
    let env = env::var("CONNORCO_ENV")
        .unwrap_or_else(|_| {
            option_env!("CONNORCO_ENV")
...
fn handle_deep_link_event(app_handle: &tauri::AppHandle, urls: Vec<String>) {
    for url in &urls {
        // Only handle connorandco schemes (connorandco://, connorandco-dev://, connorandco-staging://)
        if let Some(idx) = url.find("://") {
            let scheme = &url[..idx];
            if !scheme.contains("connorandco-app") {
                continue;
            }
            let path = &url[idx + 3..];
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Set the default app menu to restore the Connor & Co menu
