...
import type { AppRouter } from "@connorco/api/trpc/routers/_app";
import { getLocationHeaders } from "@connorco/location";
...
const getServerRequestContext = cache(async () => {
  ...
  const forcePrimary = cookieStore.get(Cookies.ForcePrimary)?.value === "true";
  ...
  client: createTRPCClient({
    links: [
      httpBatchLink({
        ...
        async headers() {
          const { session, location, forcePrimary } =
...
export async function getTRPCClient(options?: { forcePrimary?: boolean }) {
  const { session, location, forcePrimary } = await getServerRequestContext();
