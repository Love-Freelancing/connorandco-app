import type { UnifiedApp } from "@connorco/app-store/types";
import { openUrl } from "@connorco/desktop-client/core";
import { isDesktopApp } from "@connorco/desktop-client/platform";
...
import { Card, CardContent, CardHeader, CardTitle } from "@connorco/ui/card";
import {
  Carousel,
  type CarouselApi,
  CarouselContent,
  CarouselItem,
} from "@connorco/ui/carousel";
import { ScrollArea } from "@connorco/ui/scroll-area";
import { Sheet, SheetContent, SheetHeader } from "@connorco/ui/sheet";
...
export function UnifiedAppComponent({ app }: UnifiedAppProps) {
  ...
  const handleOnInitialize = async () => {
    ...
    try {
      ...
      // Handle apps with installUrl (like Connor & Co Desktop download page)
      if (app.installUrl) {
        if (isDesktopApp()) {
        ...
        <div className="px-6 pb-6 flex gap-2 mt-auto">
          ...
          {app.installUrl ? (
            <Button
              ...
              disabled={!app.active}
            >
              {app.id === "connorco-desktop" ? "Download" : "Install"}
            </Button>
          ) : app.installed ? (
            <SubmitButton
              variant="outline"
            ...
            <div className="mt-4">
              ...
              <div className="absolute bottom-4 pt-8 border-t border-border">
                <p className="text-[10px] text-[#878787]">
                  All apps on the Connor & Co App Store are open-source and
                  peer-reviewed. Connor & Co Labs AB maintains high standards but
                  doesn't endorse third-party apps. Apps published by Connor & Co are
                  officially certified. Report any concerns about app content or
                  behavior.
                </p>

                <a
                  href="mailto:support@connorandco.dev"
