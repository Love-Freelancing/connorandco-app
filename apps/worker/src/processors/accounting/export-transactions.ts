import { ACCOUNTING_ERROR_CODES, RATE_LIMITS } from "@connorco/accounting";
import {
...
interface CategorizedTransaction {
  ...
  toSyncAttachments: Array<{
    ...
    removedAttachments: Array<{ connorId: string; providerId: string | null }>;
  }>;
  ...
  private categorizeTransactions(
    ...
    for (const tx of transactions) {
      ...
      const removedAttachments = [...syncedIds]
        ...
        .map((connorId) => ({
          connorId,
          providerId: syncedMapping[connorId] ?? null,
        }));
