import {
  ...
  RATE_LIMITS,
} from "@connorco/accounting";
import {
  PROVIDER_ATTACHMENT_CONFIG,
  resolveMimeType,
  sleep,
  throttledConcurrent,
} from "@connorco/accounting/utils";
import {
 ...
 * - Tracking mappings between Connor & Co IDs and provider IDs
 */
  ...
  async process(job: Job<AccountingAttachmentSyncPayload>): Promise<{
    ...
    if (removedAttachments && removedAttachments.length > 0) {
      for (const removed of removedAttachments) {
        ...
        // (the file is gone from Connor & Co, so we shouldn't track it)
        delete currentMapping[removed.connorId];
      }
    }
