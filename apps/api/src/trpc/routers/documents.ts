...
import { isMimeTypeSupportedForProcessing } from "@connorco/documents/utils";
import { triggerJob } from "@connorco/job-client";
import { remove, signedUrl } from "@connorco/supabase/storage";
...
export const documentsRouter = createTRPCRouter({
  ...
  processDocument: protectedProcedure
    ...
    .mutation(async ({ ctx: { teamId, db }, input }) => {
      ...
      const documentsWithResolvedObjectId = input.map((item) => {
        if (item.objectId) {
          return item;
        }

        // Auto-link customer-context uploads: teamId/customers/<customerId>/...
        if (
          item.filePath.length >= 3 &&
          item.filePath[1] === "customers" &&
          uuidPattern.test(item.filePath[2] ?? "")
        ) {
          return {
            ...item,
            objectId: item.filePath[2],
          };
        }
      ...
      const supportedDocuments = documentsWithResolvedObjectId.filter((item) =>
        isMimeTypeSupportedForProcessing(item.mimetype),
      );

      const unsupportedDocuments = documentsWithResolvedObjectId.filter(
        (item) => !isMimeTypeSupportedForProcessing(item.mimetype),
      );

      if (unsupportedDocuments.length > 0) {
        const updateUnsupportedDocument = async (item: {
          filePath: string[];
          objectId?: string;
        }) => {
          for (let attempt = 1; attempt <= 3; attempt++) {
            const updated = await updateDocumentByPath(db, {
              pathTokens: item.filePath,
              teamId: teamId!,
        ...
        await Promise.all(
          unsupportedDocuments.map((item) => updateUnsupportedDocument(item)),
        );
      }
