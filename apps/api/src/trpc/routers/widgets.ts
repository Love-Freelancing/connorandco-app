...
import { widgetPreferencesCache } from "@connorco/cache/widget-preferences-cache";
...
const logger = createLoggerWithContext("trpc:widgets");
  ...
  getRunway: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      try {
        ...
        logger.error("Failed to fetch runway widget", {
          teamId,
          currency: input.currency,
  ...
  getRevenueSummary: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      try {
        ...
        logger.error("Failed to fetch revenue summary widget", {
          teamId,
          from: input.from,
          to: input.to,
          currency: input.currency,
        ...
        return {
          result: {
            totalRevenue: 0,
            currency: getFallbackCurrency(input.currency),
            revenueType: input.revenueType,
            monthCount: 0,
          },
        };
  ...
  getGrowthRate: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      try {
        ...
        logger.error("Failed to fetch growth rate widget", {
          teamId,
          from: input.from,
          to: input.to,
          currency: input.currency,
  ...
  getCashFlow: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      try {
        ...
        logger.error("Failed to fetch cash flow widget", {
          teamId,
          from: input.from,
          to: input.to,
          currency: input.currency,
  ...
  getAccountBalances: protectedProcedure
    ...
    .query(async ({ ctx: { db, teamId }, input }) => {
      try {
        ...
        logger.error("Failed to fetch account balances widget", {
          teamId,
          currency: input.currency,
          error: error instanceof Error ? error.message : String(error),
        });
