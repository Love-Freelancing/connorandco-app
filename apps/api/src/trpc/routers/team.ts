...
export const teamRouter = createTRPCRouter({
  ...
  connectionStatus: protectedProcedure.query(
    async ({ ctx: { db, teamId } }) => {
      ...
      let bankConnections: Awaited<ReturnType<typeof getBankConnections>> = [];
      let inboxAccounts: Awaited<ReturnType<typeof getInboxAccounts>> = [];

      try {
        // Fetch bank connections and inbox accounts in parallel
        [bankConnections, inboxAccounts] = await Promise.all([
