...
// Mock @connorco/db/queries with a Proxy to handle any export
...
mock.module("@connorco/db/queries", () => dbQueriesMock);

// Mock @connorco/supabase/storage
mock.module("@connorco/supabase/storage", () => ({
  signedUrl: mocks.signedUrl,
  remove: mock(() => Promise.resolve({ error: null })),
}));

// Mock @connorco/job-client
mock.module("@connorco/job-client", () => ({
...
// Mock @connorco/import
mock.module("@connorco/import", () => ({
  formatAmountValue: mocks.formatAmountValue,
}));

// Mock @connorco/invoice
mock.module("@connorco/invoice/calculate", () => ({
...
mock.module("@connorco/invoice/utils", () => ({
...
mock.module("@connorco/invoice/token", () => ({
  verify: mock(() => ({ id: "invoice-123", teamId: "test-team-id" })),
}));

mock.module("@connorco/invoice", () => ({
  DEFAULT_TEMPLATE: {},
}));
...
// Mock @connorco/db/client
mock.module("@connorco/db/client", () => ({
  db: mockDb,
  primaryDb: mockDb,
  connectDb: mock(async () => mockDb),
}));

// Mock @connorco/cache/team-cache
mock.module("@connorco/cache/team-cache", () => ({
  teamCache: {
