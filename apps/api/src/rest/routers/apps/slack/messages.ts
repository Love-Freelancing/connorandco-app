import {
  ...
  ensureBotInChannel,
} from "@connorco/app-store/slack/server";
import { logger } from "@connorco/logger";
...
export function getWelcomeMessage(isPrivateChannel = false) {
  const blocks: any[] = [
    {
      ...
      text: {
        type: "mrkdwn",
        text: "Hello there! 👋 I'm your new *Connor & Co* bot. I'll send notifications about new transactions and receipt matches in this channel.",
      },
    },
    {
      type: "section",
  ...
  if (isPrivateChannel) {
    blocks.push({
      ...
      text: {
        type: "mrkdwn",
        text: "⚠️ *Important:* This is a private channel. To enable full functionality, please invite the Connor & Co bot to this channel by typing `/invite @Connor & Co` or adding it through the channel settings.",
      },
    });
  }
    ...
    elements: [
      {
        ...
        url: "https://app.connorandco.vercel.app/apps?app=slack&settings=true",
