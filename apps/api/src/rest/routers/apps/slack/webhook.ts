...
import { getAppBySlackTeamId } from "@connorco/db/queries";
  ...
  async (c) => {
    ...
    try {
      ...
      if (type === "event_callback" && event) {
        ...
        // Use channel ID to help identify the correct Connor & Co team
        ...
        const connorandcoTeamId = slackApp.teamId;

        if (!connorandcoTeamId) {
          logger.error("Connor & Co team ID not found in app integration", {
            slackTeamId: team_id,
            appId: slackApp.id,
          });
          throw new HTTPException(400, {
