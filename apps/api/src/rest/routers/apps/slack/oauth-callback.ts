...
import { config } from "@connorco/app-store/slack";
import {
  createSlackWebClient,
  getSlackInstaller,
  publishAppHome,
} from "@connorco/app-store/slack/server";
import { createApp } from "@connorco/db/queries";
  ...
  async (c) => {
    ...
    try {
      ...
      if (createdSlackIntegration?.config) {
        ...
        // This is non-blocking - OAuth flow continues even if it fails
        // Use Slack user ID (not Connor & Co user ID) for views.publish
        ...
        publishAppHome({
          ...
          if (!errorMessage.includes("not_enabled")) {
            logger.error("Failed to publish App Home on install", {
              ...
              slackUserId,
              connorandcoUserId: parsedMetadata.data.userId,
              teamId: parsedMetadata.data.teamId,
            });
          }
