...
import { calculateTotal } from "@connorco/invoice/calculate";
import { transformCustomerToContent } from "@connorco/invoice/utils";
import { decodeJobId, getQueue, triggerJob } from "@connorco/job-client";
      ...
      data: result.data.map((invoice) => {
        ...
        return {
          ...
          bottomBlock: invoice.bottomBlock
            ...
            : null,
          pdfUrl: token
            ? `${process.env.CONNORCO_DASHBOARD_URL}/api/download/invoice?token=${token}`
            : null,
          previewUrl: token
            ? `${process.env.CONNORCO_DASHBOARD_URL}/i/${token}`
            : null,
        };
      }),
    };
